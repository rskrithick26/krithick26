...
**Important:** In Vercel, set **Root Directory** to the repository root (`.`). Do not set it to `artifacts/drugscope`. The repository is a pnpm workspace, and the Vercel build command resolves the DrugScope package from the workspace root.

Use these project settings (the included `vercel.json` already configures them):
- Install Command: `pnpm install --frozen-lockfile`
- Build Command: `pnpm --filter ./artifacts/drugscope build`
- Output Directory: `artifacts/drugscope/dist/public`

If Vercel still shows `No Output Directory named "public"`, it is using an older project configuration. Redeploy the latest commit after confirming the Root Directory is `.` and that the repository contains the root `vercel.json`.
